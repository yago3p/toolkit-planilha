...
Extraia abas de uma planilha, compile campos específicos de múltiplos arquivos, segmente dados por intervalo numérico, ou una abas inteiras de vários arquivos — tudo direto no navegador.
...
- **Compilar planilhas** — consolide campos específicos de múltiplos arquivos XLSX em uma única tabela (linha a linha), com:
...
- **Unir arquivos** — junte abas inteiras de múltiplos arquivos em um único XLSX, sem misturar os dados numa tabela (diferente do "Compilar Planilhas"):
  - Cards de arquivo colapsáveis, com contador de abas selecionadas sempre visível
  - Seleção de abas por chips clicáveis, com atalho "selecionar/nenhuma deste arquivo"
  - Busca por nome de arquivo ou de aba
  - Renomeação automática em caso de abas com nomes repetidos

Um modal de ajuda (ícone "?" no cabeçalho) explica cada uma das quatro abas e suas funções. Todas as operações rodam 100% no navegador (client-side), usando [SheetJS](https://sheetjs.com/) e [JSZip](https://stuk.github.io/jszip/) — nenhum arquivo é enviado a servidores externos.
